...
class Discriminator_CBF:
    ...
    def construct_network(self, input, safety_q): # we need to modify this
        layer_1_a = tf.layers.dense(inputs=input, units=56, activation=tf.nn.relu, name='layer1a')
        layer_2_a = tf.layers.dense(inputs=layer_1_a, units=56, activation=tf.nn.relu, name='layer2a')

        layer_1_b = tf.layers.dense(inputs=safety_q, units=8, activation=tf.nn.relu, name='layer1b')
        layer_2_b = tf.layers.dense(inputs=layer_1_b, units=8, activation=tf.nn.relu, name='layer2b')

        layer_3 = tf.layers.dense(inputs=tf.concat([layer_2_a, layer_2_b], axis=1), units=64, activation=tf.nn.relu, name='layer3') #128
    ...
    def train(self, expert_s, expert_a, agent_s, agent_a, expert_sa_p,agent_sa_p, expert_safety_q, agent_safety_q):
